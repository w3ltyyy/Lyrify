...
export const LS_UI_FONT = "spotytext_ui_font_px";
export const LS_UI_BLUR = "spotytext_ui_blur_px";
export const LS_UI_BRIGHT = "spotytext_ui_brightness";
export const LS_UI_AUTO = "spotytext_ui_auto_scroll";
export const LS_UI_DEBUG = "spotytext_ui_show_debug";
export const LS_UI_FULLSCREEN = "spotytext_ui_fullscreen";
export const LS_UI_AUTOGEN = "spotytext_ui_autogen_sync";
export const LS_UI_LINE_GAP = "spotytext_ui_line_gap";
export const LS_UI_MAX_W = "spotytext_ui_max_width";
export const LS_UI_INACTIVE_OP = "spotytext_ui_inactive_op_pct";
export const LS_UI_EDGE_FADE = "spotytext_ui_edge_fade";
export const LS_UI_VIBRANT = "spotytext_ui_vibrant";
export const LS_CONTRIBUTOR_NICKNAME = "spotytext_contributor_nickname";
...
export function createSettingsPanel(targetOverlay?: HTMLElement, onClearCache?: () => void, manualSync?: ManualSyncController) {
  const panel = h("div", { id: "spotytext-settings-panel" });
  
  const title = h("div", { id: "spotytext-settings-title" }, "Display settings");
  const closeBtn = h("button", { id: "spotytext-settings-close", type: "button" }, "Close");
  title.appendChild(closeBtn);
  panel.appendChild(title);
  
  const nav = h("div", { id: "spotytext-settings-nav" });
  const content = h("div", { id: "spotytext-settings-content" });
  const sectionWrapper = h("div", { id: "spotytext-settings-section-wrapper" });
  
  content.appendChild(sectionWrapper);
  panel.appendChild(nav);
  panel.appendChild(content);

  const generalSection = h("div", { id: "spotytext-cat-general", className: "spotytext-settings-category" });
  const appearanceSection = h("div", { id: "spotytext-cat-appearance", className: "spotytext-settings-category" });
  const syncSection = h("div", { id: "spotytext-cat-sync", className: "spotytext-settings-category" });
  ...
  const row = (label: string, control: HTMLElement, val: HTMLElement) => {
    const r = h("div", { className: "spotytext-setting-row" });
  ...
  const fontVal = h("span", { className: "spotytext-setting-val" });
  
  const blurRange = h("input", { type: "range", min: "0", max: "16", step: "1" }) as HTMLInputElement;
  const blurVal = h("span", { className: "spotytext-setting-val" });
  
  const brightRange = h("input", { type: "range", min: "75", max: "135", step: "1" }) as HTMLInputElement;
  const brightVal = h("span", { className: "spotytext-setting-val" });
  ...
  const lineGapVal = h("span", { className: "spotytext-setting-val" });

  const maxWidthRange = h("input", { type: "range", min: "480", max: "960", step: "10" }) as HTMLInputElement;
  const maxWidthVal = h("span", { className: "spotytext-setting-val" });

  const inactiveOpRange = h("input", { type: "range", min: "15", max: "55", step: "1" }) as HTMLInputElement;
  const inactiveOpVal = h("span", { className: "spotytext-setting-val" });

  const nicknameInput = h("input", { type: "text", maxLength: "32", placeholder: "Anonymous", className: "spotytext-setting-text" }) as HTMLInputElement;
  ...
  appearanceSection.appendChild(h("div", { className: "spotytext-settings-subtitle" }, "Lyrics column"));
  ...
  syncSection.appendChild(h("div", { className: "spotytext-settings-subtitle" }, "Sync engine"));
  ...
  const syncSettingsControls = (explicitTarget?: HTMLElement) => {
    ...
    (window as any).spotytext_settings = s;
    ...
    const target = explicitTarget || targetOverlay || document.getElementById("spotytext-overlay");
    if (target instanceof HTMLElement) {
        target.style.setProperty("--spotytext-line-font-size", `${s.fontPx}px`);
        target.style.setProperty("--spotytext-backdrop-blur", s.blurPx > 0 ? `blur(${s.blurPx}px)` : "none");
        target.style.setProperty("--spotytext-brightness", String(s.brightness));
        target.style.setProperty("--spotytext-lines-gap", `${s.lineGapPx}px`);
        target.style.setProperty("--spotytext-lines-max-width", `${s.maxWidthPx}px`);
        target.style.setProperty("--spotytext-line-dim-opacity", String(s.inactiveOpacityPct / 100));
        target.classList.toggle("s-fullscreen", s.fullscreen);
        target.classList.toggle("s-vibrant-enabled", s.vibrant);
        
        const scrollWrap = target.querySelector("#spotytext-scroll-wrap") || document.getElementById("spotytext-scroll-wrap");
        if (scrollWrap instanceof HTMLElement) scrollWrap.classList.toggle("s-fade-disabled", !s.edgeFade);

        const debugEl = target.querySelector("#spotytext-debug-info") || document.getElementById("spotytext-debug-info");
  ...
  const addNav = (lab: string, targetId: string) => {
    const b = h("button", { className: "spotytext-settings-cat-btn" }, lab);
    b.onclick = () => {
  ...
  addNav("General", "spotytext-cat-general");
  addNav("Appearance", "spotytext-cat-appearance");
  addNav("Sync", "spotytext-cat-sync");
